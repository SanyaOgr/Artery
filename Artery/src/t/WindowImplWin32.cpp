...
namespace engine {
	...
	WindowImplWin32::WindowImplWin32(const std::string& title, unsigned int width, unsigned int height)
	{
	}

	WindowSystemHandle WindowImplWin32::GetSystemHandle() const
	{
		return static_cast<WindowSystemHandle>(m_handle);
	}
	...
	void WindowImplWin32::registerWindowClass()
	{
		...
		s_windowClass.lpfnWndProc = &WindowImplWin32::globalOnMsgEvent;
		s_windowClass.cbClsExtra = 0;
		s_windowClass.cbWndExtra = 0;
		s_windowClass.hInstance = GetModuleHandle(nullptr);
		s_windowClass.hIcon = LoadIcon(s_windowClass.hInstance, IDI_APPLICATION);
		s_windowClass.hIconSm = s_windowClass.hIcon;
		...
		s_windowClass.lpszClassName = "WindowImplWin32";
		ASSERT(RegisterClassEx(&s_windowClass));
	}

	LRESULT WindowImplWin32::globalOnMsgEvent(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		// Associate handle and Window instance when the creation uMsg is received
		if (uMsg == WM_CREATE)
		{
		...
		WindowImplWin32* window = hWnd ? reinterpret_cast<WindowImplWin32*>(GetWindowLongPtr(hWnd, GWLP_USERDATA)) : nullptr;

		// Forward the event to the appropriate function
		if (window)
		{
			window->processMsgEvent(uMsg, wParam, lParam);

			if (window->m_callback)
				return CallWindowProc(reinterpret_cast<WNDPROC>(window->m_callback), hWnd, uMsg, wParam, lParam);
		}

		// We don't forward the WM_CLOSE uMsg to prevent the OS from automatically destroying the window
		if (uMsg == WM_CLOSE)
			return 0;
	...
	void WindowImplWin32::processMsgEvent(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
        // Don't process any uMsg until window is created
        if (m_handle == nullptr)
            return;
        ...
        case WM_SIZE:
        {
            WindowResizeEvent wre(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
            pushEvent(wre);
            break;
        }
        ...
        case WM_MOUSEWHEEL:
        {
            ...
            MouseScrolledEvent mse(GET_WHEEL_DELTA_WPARAM(wParam), GET_WHEEL_DELTA_WPARAM(wParam));
        ...
        case WM_MBUTTONDOWN:
        {
            MouseButtonPressedEvent mbpe(wParam);
        ...
        case WM_MBUTTONUP:
        {
            MouseButtonReleasedEvent mbre(wParam);
    ...
    Keyboard::Key WindowImplWin32::virtualKeyCodeToEnum(WPARAM key, LPARAM flags)
    {
        switch (key)
        {
